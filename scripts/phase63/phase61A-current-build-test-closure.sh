...
cd "$PHASE61_ROOT"
phase_setup 61A 'Build and test green closure (merged baseline compatibility)'
PHASE_STATUS=FAIL; PHASE_MESSAGE='current build/test certification failed'
trap 'code=$?; phase_finalize "$PHASE_STATUS" "$code" "$PHASE_MESSAGE"' EXIT
phase_run 'Phase 61 merged-baseline compatibility' python3 scripts/phase63/verify_phase61_compatibility.py \
  --root . --output "$PHASE61_PHASE_DIR/phase61-compatibility.json"
phase_run 'Phase 63 static contract' python3 scripts/verify_phase63_static.py
if phase_is_preflight; then
  PHASE_STATUS=PREPARED; PHASE_MESSAGE='merged-baseline build/report/flaky-test certification contracts are ready'; exit 0
fi
...
PHASE_STATUS=PASS; PHASE_MESSAGE='Maven verify passed with fresh reports, zero failures/errors and stable critical tests on merged baseline'
