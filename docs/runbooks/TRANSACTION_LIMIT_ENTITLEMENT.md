...
## Switching Transaction Limit Denials Spike
...
- Summary: Transaction limit denials exceed expected baseline
- Description: Transaction-limit denials exceed the established fifteen-minute baseline.

PromQL/expression:

```promql
switching_limits_denied_last15m > 100
```

Immediate response:

1. Confirm `SwitchingTransactionLimitDenialsSpike` is active in Prometheus and Alertmanager, including labels and receiver.
