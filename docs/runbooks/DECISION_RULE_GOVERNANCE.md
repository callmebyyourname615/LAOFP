...
## Switching Decision Rule Approval Pending
...
- Summary: Decision rule version is waiting for governance approval/deployment
- Description: A decision-rule version is awaiting governance approval or deployment.

PromQL/expression:

```promql
switching_rules_pending_approval > 0
```

Immediate response:

1. Confirm `SwitchingDecisionRuleApprovalPending` is active in Prometheus and Alertmanager, including labels and receiver.
