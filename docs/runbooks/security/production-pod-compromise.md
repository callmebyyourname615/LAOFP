...
## Switching Production Pod Exec

### Alert: `SwitchingProductionPodExec`

- Severity: `critical`
- Hold: `0m`
- Rule source: `monitoring/prometheus/phase56-security-rules.yaml`
- Summary: Exec into production pod detected
- Description: An interactive exec session was detected in a production pod and must be treated as a potential compromise.

PromQL/expression:

```promql
increase(switching_security_production_pod_exec_total[5m]) > 0
```

Immediate response:

1. Confirm `SwitchingProductionPodExec` is active in Prometheus and Alertmanager, including labels and receiver.
