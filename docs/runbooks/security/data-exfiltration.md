...
## Switching Application User D D L
...
- Summary: Application database user executed DDL
- Description: The runtime application database role executed a DDL statement and requires immediate investigation.

PromQL/expression:

```promql
increase(switching_security_db_ddl_by_application_total[5m]) > 0
```

Immediate response:

1. Confirm `SwitchingApplicationUserDDL` is active in Prometheus and Alertmanager, including labels and receiver.
