...
## Switching Control Evidence Unverified
...
- Summary: Control evidence has not been independently verified
- Description: Sealed control evidence is awaiting independent verification.

PromQL/expression:

```promql
switching_evidence_unverified > 0
```

Immediate response:

1. Confirm `SwitchingControlEvidenceUnverified` is active in Prometheus and Alertmanager, including labels and receiver.
