...
## Switching Liquidity Headroom Low

### Alert: `SwitchingLiquidityHeadroomLow`

- Severity: `critical`
- Hold: `2m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: Participant liquidity headroom below minimum
- Description: Participant liquidity headroom is below the approved minimum.

PromQL/expression:

```promql
switching_liquidity_breaches_open > 0
```

Immediate response:

1. Confirm `SwitchingLiquidityHeadroomLow` is active in Prometheus and Alertmanager, including labels and receiver.
