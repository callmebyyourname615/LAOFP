...
## Switching Synthetic Probe Failed

### Alert: `SwitchingSyntheticProbeFailed`

- Severity: `critical`
- Hold: `1m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: Synthetic payment probe failed
- Description: The controlled synthetic payment path failed within the last ten minutes.

PromQL/expression:

```promql
switching_synthetic_failures_last10m > 0
```

Immediate response:

1. Confirm `SwitchingSyntheticProbeFailed` is active in Prometheus and Alertmanager, including labels and receiver.
