# RB 09 DATABASE AND QUEUE PRESSURE
...
1. Confirm `SwitchingDatabasePoolSaturated` is active in Prometheus and Alertmanager, including labels and receiver.
...
```promql
max(switching_ops_outbox_pending{application="switching-api"}) > 100
```

Immediate response:

1. Confirm `SwitchingOutboxBacklog` is active in Prometheus and Alertmanager, including labels and receiver.
2. Compare the current value with the last known healthy period and the current release/change timeline.
3. Check application health, pod/container restarts, PostgreSQL, Kafka/outbox and dependency status relevant to this rule.
4. If financial correctness may be affected, pause the related batch/cutover and run reconciliation before resuming.
