...
## Switching Balance Mismatch Detected

### Alert: `SwitchingBalanceMismatchDetected`

- Severity: `critical`
- Hold: `0m`
- Rule source: `monitoring/prometheus/phase56-data-integrity-rules.yaml`
- Summary: Balance mismatch detected
- Description: Reconciliation detected a non-zero balance mismatch that may indicate missing or duplicate financial postings.

PromQL/expression:

```promql
switching_reconciliation_balance_mismatch_total > 0
```

Immediate response:

1. Confirm `SwitchingBalanceMismatchDetected` is active in Prometheus and Alertmanager, including labels and receiver.
