...
## Switching Crypto Rotation Overdue
...
- Summary: Cryptographic asset rotation is overdue
- Description: A governed cryptographic asset rotation is overdue.

PromQL/expression:

```promql
switching_crypto_rotation_overdue > 0
```

Immediate response:

1. Confirm `SwitchingCryptoRotationOverdue` is active in Prometheus and Alertmanager, including labels and receiver.
