...
## Switching Decommission Plan Overdue
...
- Summary: Controlled decommission plan passed its effective time
- Description: A controlled-decommission plan passed its effective time without completion.

PromQL/expression:

```promql
switching_decommission_overdue > 0
```

Immediate response:

1. Confirm `SwitchingDecommissionPlanOverdue` is active in Prometheus and Alertmanager, including labels and receiver.
