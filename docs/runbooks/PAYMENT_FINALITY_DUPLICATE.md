...
## Switching Finality Reversal Pending
...
- Summary: Exceptional payment reversal is pending
- Description: An exceptional payment-finality reversal is pending beyond thirty minutes.

PromQL/expression:

```promql
switching_finality_reversals_pending > 0
```

Immediate response:

1. Confirm `SwitchingFinalityReversalPending` is active in Prometheus and Alertmanager, including labels and receiver.
