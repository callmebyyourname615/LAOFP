...
## Switching Regulatory Report Overdue

### Alert: `SwitchingRegulatoryReportOverdue`

- Severity: `critical`
- Hold: `15m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: Regulatory report submission overdue
- Description: A required regulatory report has missed its submission deadline.

PromQL/expression:

```promql
switching_regulatory_reports_overdue > 0
```

Immediate response:

1. Confirm `SwitchingRegulatoryReportOverdue` is active in Prometheus and Alertmanager, including labels and receiver.
