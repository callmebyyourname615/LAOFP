...
## Switching Third Party Circuit Open
...
- Summary: Critical third-party dependency circuit is open
- Description: A critical third-party dependency circuit remains open.

PromQL/expression:

```promql
switching_dependencies_circuit_open > 0
```

Immediate response:

1. Confirm `SwitchingThirdPartyCircuitOpen` is active in Prometheus and Alertmanager, including labels and receiver.
