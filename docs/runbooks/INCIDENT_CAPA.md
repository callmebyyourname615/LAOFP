...
## Switching Corrective Action Overdue

### Alert: `SwitchingCorrectiveActionOverdue`

- Severity: `warning`
- Hold: `30m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: Incident corrective action is overdue
- Description: An incident corrective action has passed its committed due date.

PromQL/expression:

```promql
switching_incident_actions_overdue > 0
```

Immediate response:

1. Confirm `SwitchingCorrectiveActionOverdue` is active in Prometheus and Alertmanager, including labels and receiver.
