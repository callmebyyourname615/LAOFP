...
## Switching Governed Fx Rate Missing

### Alert: `SwitchingGovernedFxRateMissing`

- Severity: `critical`
- Hold: `2m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: No valid governed FX rate
- Description: At least one enabled currency pair has no valid approved FX rate.

PromQL/expression:

```promql
switching_fx_pairs_without_valid_rate > 0
```

Immediate response:

1. Confirm `SwitchingGovernedFxRateMissing` is active in Prometheus and Alertmanager, including labels and receiver.
