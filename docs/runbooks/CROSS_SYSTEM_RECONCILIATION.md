...
## Switching Continuous Reconciliation Failed

### Alert: `SwitchingContinuousReconciliationFailed`

- Severity: `critical`
- Hold: `5m`
- Rule source: `monitoring/prometheus/phase56-data-integrity-rules.yaml`
- Summary: Continuous reconciliation stale or failed
- Description: The continuous reconciliation job has not completed successfully within the allowed freshness window.

PromQL/expression:

```promql
switching_reconciliation_last_success_timestamp_seconds < time() - 1800
```

Immediate response:

1. Confirm `SwitchingContinuousReconciliationFailed` is active in Prometheus and Alertmanager, including labels and receiver.
