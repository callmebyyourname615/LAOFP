...
## Switching Certificate Expiring Soon

### Alert: `SwitchingCertificateExpiringSoon`

- Severity: `warning`
- Hold: `15m`
- Rule source: `monitoring/prometheus/phase33-42-alert-rules.yaml`
- Summary: Participant certificate expires within 30 days
- Description: A participant certificate expires within 30 days and requires coordinated rotation.

PromQL/expression:

```promql
switching_participant_certificates_expiring_30d > 0
```

Immediate response:

1. Confirm `SwitchingCertificateExpiringSoon` is active in Prometheus and Alertmanager, including labels and receiver.
