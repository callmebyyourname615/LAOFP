...
|---|---|---|---|
| a | Flyway version assertion ค้างที่ "84" | `V83CleanInstallCertificationIntegrationTest`<br>`MigrationApplicationIntegrationTest`<br>`V83PayloadSha256SchemaAlignmentIntegrationTest` | ตอนนี้ migration ล่าสุดคือ V96 ไม่ใช่ V84 |
...
# In each test file, change isEqualTo("84") → isEqualTo("96")
#                       change isEqualTo(84L) → isEqualTo(93L)   (93 = count of migrations: 87 + 6)
...
# Expected: Tests run: 451, Failures: 0, Errors: 0
```
...
### Why critical
ใน git history มี credentials default (`change_me`, `switching_app_password_change_me`) — anyone with clone access เห็น เป็น GDPR/PCI breach ได้

### 6 credentials ที่ต้อง rotate

| Credential | ปัจจุบัน (dev default) | Update ที่ไหน |
|---|---|---|
| `POSTGRES_PASSWORD` | `switching_postgres_password_change_me` | Vault prod + dev `.env` |
| `REPLICATION_PASSWORD` | `switching_replicator_password_change_me` | Vault prod + dev `.env` |
| `DB_APP_PASSWORD` | `switching_app_password_change_me` | Vault prod + dev `.env` |
| `FLYWAY_PASSWORD` | `switching_flyway_password_change_me` | Vault prod + dev `.env` |
| `ARCHIVE_POSTGRES_PASSWORD` | `switching_archive_postgres_password_change_me` | Vault prod + dev `.env` |
| `MINIO_ROOT_PASSWORD` | `switching_minio_password_change_me` | Vault prod + dev `.env` |
...
# Step 3 — Force-push purged git history
# Coordinate with team (freeze writes ~30 นาที)
# Use BFG Repo-Cleaner or git filter-repo:
git filter-repo --replace-text <(grep "=" /tmp/new-prod-secrets.env | sed 's/=/==>REDACTED_/')
...
- [performance/scripts/k6/sustained10k.js](performance/scripts/k6/sustained10k.js) — k6 script ramp 0→10K over 5 นาที, hold 60 นาที
- [performance/scripts/k6/burst20k.js](performance/scripts/k6/burst20k.js) — ramp 10K→20K over 30 วินาที, hold 15 นาที
- [performance/scripts/k6/soak8h.js](performance/scripts/k6/soak8h.js) — 5K constant for 8h
- Sample k6 script structure:
  ...
  export const options = {
    stages: [
      ...
      { duration: '60m', target: 10000 },  // hold
