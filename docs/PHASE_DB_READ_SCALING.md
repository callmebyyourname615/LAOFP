...
## Switching Reporting Aggregate Stale

### Alert: `SwitchingReportingAggregateStale`

- Severity: `warning`
- Hold: `15m`
- Rule source: `monitoring/prometheus/database-lifecycle-rules.yaml`
- Summary: Reporting aggregates have not refreshed for more than two hours
- Description: Dashboard data may be stale; inspect aggregation scheduler and replica lag.

PromQL/expression:

```promql
max(switching_database_reporting_refresh_age_seconds) > 7200
```

Immediate response:

1. Confirm `SwitchingReportingAggregateStale` is active in Prometheus and Alertmanager, including labels and receiver.
